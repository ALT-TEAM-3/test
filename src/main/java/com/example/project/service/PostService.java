...
public class PostService {
    ...
    public PostResponse create(String email, PostCreateRequest request) {
        return PostResponse.from(postRepository.save(new Post(currentUser(email), request.title(), request.content())));
    }
    ...
    @Transactional
    public PostResponse update(Long id, String email, PostUpdateRequest request) {
        Post post = findPost(id);
        verifyOwner(post, email);
    ...
    private void verifyOwner(Post post, String email) {
        if (!post.getUser().getEmail().equals(email)) throw new CustomException(ErrorCode.FORBIDDEN);
    }
}
